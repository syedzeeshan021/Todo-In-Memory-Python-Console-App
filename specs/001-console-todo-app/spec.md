...
- Q: How should the confirmation prompt work when deleting a task? → A: Standard "Are you sure? (y/N)" format with case-insensitive response
- Q: What type of error messages should be displayed to users when they enter invalid input? → A: Clear, descriptive error messages that guide the user on how to correct their input
- Q: Should the console commands be case-sensitive or case-insensitive? → A: Commands should be case-insensitive to improve usability (e.g., "ADD", "add", "Add" all work)
...
- **FR-008**: Confirmation prompt for deletion MUST use standard "Are you sure? (y/N)" format with case-insensitive response
- **FR-009**: System MUST maintain sequential ID assignment without reuse after deletion
- **FR-010**: System MUST run in an interactive console loop until user exits via 'quit' command or Ctrl+C
- **FR-011**: System MUST handle all invalid user input gracefully without crashing
- **FR-012**: System MUST never expose stack traces to the user - all exceptions must be caught at UI boundary
- **FR-013**: Error messages MUST be clear and descriptive, guiding the user on how to correct their input
- **FR-014**: Console commands MUST be case-insensitive to improve usability (e.g., "ADD", "add", "Add" all work)
